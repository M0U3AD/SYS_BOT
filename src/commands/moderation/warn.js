...
module.exports = {
  ...
  async execute(message, args) {
    ...
    if (args[0] === 'list') {
      ...
      const list = warns.map((w, i) => {
        return '**' + (i + 1) + '.** ' + w.reason + ' — by <@' + w.moderatorId + '> on ' + new Date(w.date).toLocaleDateString();
      }).join('\n');
    ...
    const msg = '**' + member.user.tag + '** has been warned.\n**Reason:** ' + reason + '\n**Total Warnings:** ' + total;
    message.reply({ embeds: [successEmbed('Member Warned', msg)] });
    ...
    if (config.logging.modLogChannelId) {
      ...
      if (ch) {
        const logMsg = '**User:** ' + member.user.tag + '\n**Moderator:** ' + message.author.tag + '\n**Reason:** ' + reason + '\n**Total:** ' + total;
        ch.send({ embeds: [successEmbed('Member Warned', logMsg)] });
      }
    }
  ...
  async slashExecute(interaction, client) {
    ...
    const msg = '**' + user.tag + '** has been warned.\n**Reason:** ' + reason + '\n**Total Warnings:** ' + total;
    interaction.reply({ embeds: [successEmbed('Member Warned', msg)] });
